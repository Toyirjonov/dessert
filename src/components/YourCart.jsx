...
import { removeDessert } from "../app/featues/dessertsSlice";
import { Order } from "./Order";

function YourCart({ dessert }) {
  ...
  return (
    <div className="menu-right">
      <h2 className="menu-right-title">Your Cart ({totalAmount})</h2>
      {desserts.length === 0 ? (
        ...
        <div className="cart-content">
          <div className="cart-items">
            {desserts.map((cartdesserts) => (
              <div key={cartdesserts.id} className="cart-item">
                <div className="cart-item-info">
                  <h4 className="cart-item-name">{cartdesserts.name}</h4>
                  <div className="cart-item-details">
                    <span className="cart-item-quantity">
                      {cartdesserts.amount}x
                    </span>
                    <span className="cart-item-price">
                      @ {formatNumber(cartdesserts.price)}
                    </span>
                    <span className="cart-item-total">
                      {formatNumber(cartdesserts.price * cartdesserts.amount)}
                    </span>
                  </div>
                </div>
                <button
                  className="remove-item-btn"
                  onClick={() => dispatch(removeDessert(cartdesserts.id))}
                  aria-label={`Remove ${cartdesserts.name} from cart`}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="10"
          ...
          <div className="carbon-neutral">
            ...
            <p className="carbon-neutral-title">
              This is a <span>carbon-neutra</span> delivery
            </p>
          </div>

          <button className="confirm-order-btn" onClick={() => setOrder(true)}>
            Confirm Order
          </button>
        </div>
      )}
      {order && <Order desserts={dessert} setOrder={setOrder} />}
    </div>
  );
}
